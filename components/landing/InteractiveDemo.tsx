...
const TEXT_COLOR = '#fdf1ea';
const MUTED_TEXT = '#cba696';
const BORDER_COLOR = '#4a3327';
const FONT = 'Inter, system-ui, -apple-system, sans-serif';

function ax() {
  return {
    axisLabel: { color: MUTED_TEXT, fontSize: 11, fontFamily: FONT },
    axisLine: { lineStyle: { color: BORDER_COLOR } },
    splitLine: { lineStyle: { color: BORDER_COLOR, type: 'dashed' as const } },
    axisTick: { show: false },
  };
}
...
const SCENARIOS: Scenario[] = [
  {
    ...
    option: {
      ...
      tooltip: { trigger: 'axis', backgroundColor: '#2a1a10', borderColor: BORDER_COLOR, textStyle: { color: TEXT_COLOR, fontSize: 12 } },
      legend: { data: ['Software', 'Services', 'Hardware'], top: 8, textStyle: { color: MUTED_TEXT, fontSize: 11 } },
      grid: { top: 44, right: 16, bottom: 28, left: 46 },
      xAxis: { type: 'category', data: ['Q1', 'Q2', 'Q3', 'Q4'], ...ax() },
      yAxis: { type: 'value', ...ax(), axisLabel: { color: MUTED_TEXT, fontSize: 11, fontFamily: FONT, formatter: '${value}' }, axisLine: { show: false } },
      series: [
    ...
    messages: [
      { role: 'user', content: 'Show quarterly revenue by product line.' },
      { role: 'assistant', content: 'Grouped bar chart ready. Software grew 41% Q1→Q4. Hardware is declining steadily.' },
      { role: 'user', content: 'Sort by Q4 and make it horizontal.' },
      { role: 'assistant', content: 'Done — reordered by Q4 revenue, bars now horizontal. Want a target line or different palette?' },
    ],
  },
  {
    ...
    option: {
      backgroundColor: 'transparent',
      tooltip: { trigger: 'axis', backgroundColor: '#2a1a10', borderColor: BORDER_COLOR, textStyle: { color: TEXT_COLOR, fontSize: 12 } },
      legend: { data: ['Total Users', 'New Users'], top: 8, textStyle: { color: MUTED_TEXT, fontSize: 11 } },
      grid: { top: 44, right: 16, bottom: 28, left: 46 },
      xAxis: { type: 'category', data: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], ...ax(), boundaryGap: false },
      yAxis: { type: 'value', ...ax(), axisLine: { show: false } },
      series: [
        { name: 'Total Users', type: 'line', smooth: true, data: [1200, 1800, 2900, 4100, 5800, 7200, 9100, 11400, 14200, 17800, 22100, 28400], itemStyle: { color: PALETTE[0] }, lineStyle: { color: PALETTE[0], width: 2.5 }, areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(251,146,60,0.3)' }, { offset: 1, color: 'rgba(251,146,60,0.02)' }] } }, symbolSize: 5 },
    ...
    messages: [
      { role: 'user', content: 'Plot monthly active users over the past year.' },
      { role: 'assistant', content: 'Your MAU grew from 1.2K to 28.4K — a 23x increase. Exponential growth kicked in around August.' },
      { role: 'user', content: 'Add new user signups per month.' },
      { role: 'assistant', content: 'Dashed line shows new signups. Retention is strong — total users grow faster than new signups alone.' },
    ],
  },
  {
    ...
    option: {
      backgroundColor: 'transparent',
      tooltip: { trigger: 'item', backgroundColor: '#2a1a10', borderColor: BORDER_COLOR, textStyle: { color: TEXT_COLOR, fontSize: 12 }, formatter: '{b}: {c} ({d}%)' },
      legend: { orient: 'horizontal', bottom: 4, textStyle: { color: MUTED_TEXT, fontSize: 11 } },
      series: [{
        type: 'pie', radius: ['38%', '66%'], center: ['50%', '48%'], avoidLabelOverlap: true,
        itemStyle: { borderRadius: 5, borderColor: 'transparent', borderWidth: 2 },
        label: { show: true, color: MUTED_TEXT, fontSize: 11, formatter: '{d}%' },
        labelLine: { lineStyle: { color: BORDER_COLOR } },
    ...
    messages: [
      { role: 'user', content: 'Show customer satisfaction survey as a donut chart.' },
      { role: 'assistant', content: '72% are satisfied or very satisfied. Only 10.6% reported dissatisfaction — a strong signal overall.' },
      { role: 'user', content: 'Highlight the dissatisfied segments.' },
      { role: 'assistant', content: 'I can pull out those slices or use a contrasting red. Say the word and I\'ll update the chart.' },
    ],
  },
];
  ...
  return (
    <div className="rounded-2xl border border-edge overflow-hidden bg-surface-0">
      ...
      <div className="flex flex-col lg:flex-row" style={{ minHeight: 380 }}>
        {/* Chat */}
        <div className="lg:w-[340px] xl:w-[380px] shrink-0 border-r border-edge flex flex-col">
